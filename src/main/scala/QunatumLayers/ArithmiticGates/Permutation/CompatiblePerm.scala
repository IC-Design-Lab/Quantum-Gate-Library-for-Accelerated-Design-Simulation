...
class CompatiblePerm( val num_of_qubits: Int,
                      val bitwidth: Int,
                      val target0: Int,
                      val target1: Int
  ...
  for (i <- 0 until dim) {
    perm.io.in_ket(i)(0) := io.in_QSV(i)(0).asUInt
    perm.io.in_ket(i)(1) := io.in_QSV(i)(1).asUInt
  }
  ...
  for (i <- 0 until dim) {
    io.out_QSV(i)(0) := perm.io.out_ket(i)(0).asSInt
    io.out_QSV(i)(1) := perm.io.out_ket(i)(1).asSInt
  }
  //io.out_QSV := outSInt
